...
var Actor = (function () {
    function Actor(name, code, game) {
        ...
        this.defaultKeyName = 'right';
        this.angleNamePatterns = {
            'left': new Vector(-1, 0),
            'right': new Vector(1, 0),
            'up': new Vector(0, -1),
            'down': new Vector(0, 1)
        };
        this.defaultAngleName = 'right';
        this.accelerateNamePatterns = {
            'very_fast': 0.3,
            'fast': 0.1,
            'normal': 0.03,
            'slow': 0.01,
        ...
        this.defaultAccelerateName = 'normal';
        this.moveNamePatterns = {
            'very_fast': 3,
            'fast': 2,
            'normal': 1,
            'slow': 0.5,
        ...
        this.defaultMoveName = 'normal';
        this.screen = game.screen;
        var ci = game.actorNames.indexOf(name) - 1;
        if (ci >= 0) {
            this.colorIndex = ci;
        }
    }
    ...
    Actor.prototype.parse = function (currentCode) {
        var _this = this;
        var stack = [];
        this.parseCount++;
        ...
        for (var i = 0; i < 32; i++) {
            ...
            switch (c) {
                case 'if':
                    var cond = currentCode.shift();
                    var condResult = this.parse(cond);
                    var thenCode = currentCode.shift();
                    var elseCode = currentCode.shift();
                    if (condResult) {
                        this.parse(thenCode);
                    }
                    else {
                ...
                case 'exists':
                case 'not_exists':
                    var actorName = this.parse(currentCode.shift());
                    if (c === 'exists') {
                ...
                case 'random':
                case 'interval':
                    var freqName = this.parse(currentCode.shift());
                    var freq = this.freqNamePatterns[freqName];
                    if (freq == null) {
                ...
                case 'key':
                    ...
                    var keyName = this.parse(currentCode.shift());
                    var kp = this.keyNamePatterns[keyName];
                    if (kp == null) {
                        kp = this.keyNamePatterns[this.defaultKeyName];
                    }
                    this.resultValue = _.some(kp, function (k) { return _this.game.isKeyDown[k]; });
                    break;
                case 'touch':
                    var targetName = this.parse(currentCode.shift());
                    this.resultValue = this.checkTouch(targetName);
                    break;
                case 'select':
                    var selectedIndex = this.game.random.getInt(currentCode.length);
                    this.parse(currentCode[selectedIndex]);
                    currentCode = [];
                    break;
                case 'spawn':
                    var spawningActorName = this.parse(currentCode.shift());
                    var actor = this.game.addActor(spawningActorName);
                    if (actor != null) {
    ...
    Actor.prototype.checkTouch = function (targetName) {
        var _this = this;
        if (targetName == null) {
            return false;
        }
        ...
        var ti = this.game.actorNames.indexOf(targetName);
        if (ti >= 0) {
            this.touchedActor = null;
            var x_1 = Math.floor(this.pos.x);
            var y_1 = Math.floor(this.pos.y);
            return _.some(this.game.actors, function (a) {
                var isTouched = (a !== _this && a.name === targetName &&
                    Math.floor(a.pos.x) === x_1 && Math.floor(a.pos.y) === y_1);
                if (isTouched) {
                    _this.touchedActor = a;
                }
                return isTouched;
            });
        }
...
Object.defineProperty(exports, "__esModule", { value: true });
var _ = __webpack_require__(0);
var actor_1 = __webpack_require__(2);
var random_1 = __webpack_require__(1);
var Game = (function () {
    function Game(screen, isKeyDown, randomSeed, autoPressingRandomSeed) {
        if (randomSeed === void 0) { randomSeed = null; }
        if (autoPressingRandomSeed === void 0) { autoPressingRandomSeed = null; }
        this.screen = screen;
        this.isKeyDown = isKeyDown;
        this.maxActorCount = 100;
        this.originalCode = {};
        this.codes = {};
        this.ticks = -1;
        this.score = 0;
        ...
        this.autoPressingKeys = [37, 38, 39, 40];
        this.autoPressingRandom = new random_1.default();
        this.actorNames = ['stage', 'player', 'enemy', 'shot', 'bullet'];
        if (randomSeed != null) {
        ...
        if (autoPressingRandomSeed != null) {
            ...
            this.isKeyDown = _.times(256, function () { return false; });
        }
        this.update = this.update.bind(this);
    }
    Game.prototype.begin = function (gameCode) {
        var _this = this;
        this.originalCode = _.cloneDeep(gameCode);
        var code = _.cloneDeep(gameCode);
        code.splice(0, 2); // Remove 'game', [name]
        _.forEach(code, function (ac) {
            var name = ac[1];
            if (_.some(_this.actorNames, function (an) { return an === name; })) {
    ...
    Game.prototype.addActor = function (name) {
        ...
        var actor = new actor_1.default(name, this.codes[name], this);
        this.actors.push(actor);
        return actor;
    };
    Game.prototype.getActors = function (name) {
        return _.filter(this.actors, function (a) { return a.name === name; });
    };
    Game.prototype.update = function () {
        ...
        if (this.isAutoPressing) {
            if (this.autoPressingRandom.get() < 0.2) {
                _.forEach(this.autoPressingKeys, function (k) {
                    _this.isKeyDown[k] = false;
                });
                if (this.autoPressingRandom.get() > 0.2) {
                    this.isKeyDown[this.autoPressingKeys[this.autoPressingRandom.getInt(this.autoPressingKeys.length)]] = true;
                }
            }
        }
        this.screen.clear();
    ...
    Game.prototype.diff = function (otherGame) {
        return {
...
var _ = __webpack_require__(0);
var Screen = (function () {
    function Screen(hasDom, mode, width, height) {
        if (hasDom === void 0) { hasDom = true; }
        if (mode === void 0) { mode = 'generated'; }
        ...
        this.pixels = _.times(width, function () { return _.times(height, function () { return -1; }); });
        if (!hasDom) {
            return;
        }
        ...
        var styleSize = mode === 'generated' ? 120 : 300;
    ...
    Screen.prototype.setOnButtonClicked = function (handler) {
    ...
    Screen.prototype.clear = function () {
        ...
        _.times(this.width, function (x) { return _.times(_this.height, function (y) {
            _this.pixels[x][y] = -1;
        }); });
    };
    Screen.prototype.setPoint = function (x, y, colorIndex) {
        if (colorIndex === void 0) { colorIndex = -1; }
        var px = Math.floor(x);
        var py = Math.floor(y);
        if (colorIndex === -1 ||
    ...
    Screen.prototype.diff = function (otherScreen) {
        var _this = this;
        var score = 0;
        _.times(this.width, function (x) {
            _.times(_this.height, function (y) {
                if (_this.pixels[x][y] !== otherScreen.pixels[x][y]) {
                    score++;
                }
            });
        });
    ...
    Screen.prototype.showStatus = function (text) {
        if (!this.hasDom) {
            return;
        }
...
Object.defineProperty(exports, "__esModule", { value: true });
var parseSE = __webpack_require__(7);
...
var codeCount = 100;
var aliveCount = 10;
var crossoverCount = 5;
var fitnessCalcTicks = 100;
var version = '1';
var baseCodeCount;
var baseCodeNames;
var baseCodes;
var baseCodesDiffParams;
var loadedBaseCodeCount = 0;
var codeSeed;
var codes;
var codeIndex;
var codesWithFitness;
var fitnessIndex;
var random;
var isKeyDown = _.times(256, function () { return false; });
var games = [];
var isLiked = [];
...
function initEventHandlers() {
    ...
    document.getElementById('generate_from_liked').onclick = function () {
        goToNextGeneration();
    };
}
function loadGameList() {
    loadFile('games.txt', function (text) {
        baseCodeNames = _.filter(text.split('\n'), function (n) { return n.length > 0; });
        baseCodeNames = _.map(baseCodeNames, function (n) { return n.trim(); });
        baseCodeCount = baseCodeNames.length;
        baseCodes = _.times(baseCodeCount, function () { return null; });
        _.forEach(baseCodeNames, loadCode);
    });
}
function loadCode(name, index) {
    loadFile("codes/" + name, function (text) {
        var parsed = parseSE(text);
...
function beginGenerating(randomSeed) {
    ...
    enableButtons(false);
    endGames();
    codeSeed = randomSeed != null ? randomSeed : new random_1.default().getToMaxInt();
    random = new random_1.default().setSeed(codeSeed);
    var codeOffset = random.getInt(baseCodeCount);
    codes = _.times(codeCount, function (i) {
        return {
...
function beginGeneratedGames() {
    resetUrl();
    games = _.map(codes, function (c) { return beginGame(c); });
    _.forEach(isLiked, function (il, i) {
...
function beginGame(code, mode) {
    if (mode === void 0) { mode = 'generated'; }
    var fitness = Math.floor(code.fitness);
    var screen = new screen_1.default(true, mode);
    var game = new game_1.default(screen, isKeyDown);
    if (mode === 'generated') {
        screen.setOnButtonClicked(function () {
            _.forEach(games, function (g, i) {
...
function crossoverCodes() {
    _.times(codeCount * crossoverCount, function (i) {
...
function endSelectingCodes() {
    var ci = 0;
    _.times(codeCount - aliveCount, function () {
        var nci = ci + 1;
        if (nci >= codesWithFitness.length) {
            nci = 0;
        }
        var si = (codesWithFitness[ci].fitness > codesWithFitness[nci].fitness) ? nci : ci;
    ...
    isLiked = _.times(aliveCount, function () { return true; });
    beginGeneratedGames();
}
function addFitnessToCode() {
    showInfo("Generating... " + fitnessIndex + " / " + codeCount);
    var code = codes[fitnessIndex].code;
    var fitness = calcFitness(code);
    codesWithFitness.push({ code: code, fitness: fitness });
    fitnessIndex++;
...
var diffParams = ['screen', 'score', 'miss'];
function calcFitness(code) {
    var diff = calcDiff(code);
    if (diff == null) {
        return 0;
    }
    var fitness = 0;
    _.forEach(diffParams, function (p) {
        var dp = baseCodesDiffParams[p];
...
function calcDiff(code) {
    var gameCount = 8;
    var games = _.times(gameCount, function (i) {
        var game = new game_1.default(new screen_1.default(false), null, 0, i);
        game.begin(code);
        return game;
    });
    var result = {};
    ...
    var _loop_1 = function (i) {
        ...
        for (var j = 0; j < gameCount - 1; j++) {
            var _loop_2 = function (k) {
                var diff = games[j].diff(games[k]);
                _.forEach(diffParams, function (p) {
                    result[p] += diff[p];
                });
            };
            for (var k = j + 1; k < gameCount; k++) {
                _loop_2(k);
            }
        }
    };
    for (var i = 0; i < fitnessCalcTicks; i++) {
...
function calcBaseCodesDiffParams() {
    ...
    var codeCount = 0;
    _.forEach(baseCodes, function (c) {
        var diff = calcDiff(c);
        if (diff == null) {
            return;
        }
...
function fitnessValue(v, min, center, max) {
    if (v <= min || v >= max) {
        return 0;
    }
    if (v < center) {
...
function crossover(codeIndex) {
    var p1 = getCodePart(codes[codeIndex].code);
    var p2 = getCodePart(codes[random.getInt(codeCount)].code);
    p1.parent.splice(p1.index, 0, _.cloneDeep(p2.parent[p2.index]));
    p2.parent.splice(p2.index, 1);
}
function getCodePart(code, targetDepth, depth) {
    if (targetDepth === void 0) { targetDepth = 1; }
    if (depth === void 0) { depth = 0; }
    var ci = random.getInt(code.length);
    var part;
...
function enableButtons(isEnabled, buttonIds) {
    if (isEnabled === void 0) { isEnabled = true; }
    if (buttonIds === void 0) { buttonIds = ['generate', 'generate_from_liked']; }
    _.forEach(buttonIds, function (id) {
...
function saveAsUrl(code) {
    var baseUrl = window.location.href.split('?')[0];
    var codeStr = LZString.compressToEncodedURIComponent(JSON.stringify(code));
    var url = baseUrl + "?v=" + version + "&c=" + codeStr;
